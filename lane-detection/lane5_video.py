...
def roi(image):
    ...
    polygon = np.array([[
        ...
        (width*0.6, height*0.76),      # Mid-right point
...
def filter_lanes(lines, image_width):
    ...
    for line in lines:
        ...
        if slope < 0 and mid_x > image_width * 0.4 and mid_x < image_width * 0.4:
            left_lines.append(line)
        elif slope > 0 and mid_x > image_width * 0.4 and mid_x < image_width * 0.6:
...
cap = cv.VideoCapture('test_video_3.mp4')
while(cap.isOpened()):
    ret ,frame=cap.read()
